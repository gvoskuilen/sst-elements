// Copyright 2009-2026 NTESS. Under the terms
// of Contract DE-NA0003525 with NTESS, the U.S.
// Government retains certain rights in this software.
//
// Copyright (c) 2009-2026, NTESS
// All rights reserved.
//
// Portions are copyright of other developers:
// See the file CONTRIBUTORS.TXT in the top level directory
// of the distribution for more information.
//
// This file is part of the SST software package. For license
// information, see the LICENSE file in the top level directory of the
// distribution.

#include <sst_config.h>

#include "decoder/vriscv64decoder.h"

#include "inst/vinstall.h"

using namespace SST::Vanadis;

VanadisRISCV64Decoder::VanadisRISCV64Decoder(ComponentId_t id, Params& params, SST::Output* output) : VanadisDecoder(id, params, output)
{
    // we need TWO additional registers for AMO microcode operations, RISC-V has 32 + 2 int for our micro-code.
    options_ = new VanadisDecoderOptions(static_cast<uint16_t>(0), 35, 32, 2, VANADIS_REGISTER_MODE_FP64);

    // See if we get an entry point the sub-component says we have to use
    // if not, we will fall back to ELFVanadisFP2FPInstruction reading at the core level to work this
    // out
    setInstructionPointer(params.find<uint64_t>("entry_point", 0));

    fatal_decode_fault_ = params.find<bool>("halt_on_decode_fault", false);

}

VanadisRISCV64Decoder::~VanadisRISCV64Decoder() {}

void VanadisRISCV64Decoder::setStackPointer( VanadisISATable* isa_tbl, VanadisRegisterFile* regFile, const uint64_t start_stack_address )
{
    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(
        CALL_INFO, 16, 0, "-> Setting SP to (64B-aligned):          %" PRIu64 " / 0x%0" PRI_ADDR "\n", start_stack_address, start_stack_address);
    #endif

    // Per RISCV Assembly Programemr's handbook, register x2 is for stack pointer
    const int16_t sp_phys_reg = isa_tbl->getIntPhysReg(2);

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, 0, "-> Stack Pointer (r29) maps to phys-reg: %" PRIu16 "\n", sp_phys_reg);
    #endif

    // Setup the initial stack pointer
    regFile->setIntReg<uint64_t>(sp_phys_reg, start_stack_address);
}

void VanadisRISCV64Decoder::setArg1Register( VanadisISATable* isa_tbl, VanadisRegisterFile* regFile, const uint64_t value )
{
    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> Setting argument 1 register to (64B-aligned): %" PRIu64 " / 0x%0" PRI_ADDR "\n", value, value);
    #endif

    const int16_t sp_phys_reg = isa_tbl->getIntPhysReg(10);

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> argument 1 (r10) maps to phys-reg: %" PRIu16 "\n", sp_phys_reg);
    #endif

    regFile->setIntReg(sp_phys_reg, value);
}

void VanadisRISCV64Decoder::setReturnRegister( VanadisISATable* isa_tbl, VanadisRegisterFile* regFile, const uint64_t value )
{
    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> Setting register 10 to (64B-aligned): %" PRIu64 " / 0x%0" PRI_ADDR "\n", value, value);
    #endif

    const int16_t sp_phys_reg = isa_tbl->getIntPhysReg(10);

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> r10 maps to phys-reg: %" PRIu16 "\n", sp_phys_reg);
    #endif

    regFile->setIntReg(sp_phys_reg, value);
}


void VanadisRISCV64Decoder::setThreadPointer( VanadisISATable* isa_tbl, VanadisRegisterFile* regFile, const uint64_t value )
{
    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> Setting thread pointer to (64B-aligned): %" PRIu64 " / 0x%0" PRI_ADDR "\n", value, value);
    #endif

    const int16_t sp_phys_reg = isa_tbl->getIntPhysReg(4);

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, VANADIS_DBG_DECODER_FLG, "-> Thread Pointer (r4) maps to phys-reg: %" PRIu16 "\n", sp_phys_reg);
    #endif

    regFile->setIntReg(sp_phys_reg, value);
}

bool VanadisRISCV64Decoder::tick(uint64_t cycle)
{
    cycle_count_ = cycle;

    bool success = false;

    VanadisInstructionBundle* bundle = ins_loader->getBundleAt(ip);
    if ( bundle ) {
        // We have the instruction in our micro-op cache
        stat_uop_hit_->addData(1);

        // Do we have enough space in the ROB to push the micro-op bundle into the queue?
        if ( bundle->getInstructionCount() < (thread_rob->capacity() - thread_rob->size()) ) {
            #ifdef VANADIS_BUILD_DEBUG
            if (output_->getVerboseLevel() >= 16) {
                output_->verbose(CALL_INFO, 16, 0,
                    "%" PRIu32 ": ----> Attempt decode ip=0x%" PRI_ADDR ": uop cache hit, loading %" PRIu32 " entries.\n",
                    hw_thr, ip, bundle->getInstructionCount());
            }
            #endif
            bool bundle_has_branch = false;

            for ( uint32_t i = 0; i < bundle->getInstructionCount(); ++i ) {
                VanadisInstruction* next_ins = bundle->getInstructionByIndex(i);

                if ( next_ins->getInstFuncType() == INST_BRANCH ) {
                    VanadisSpeculatedInstruction* next_spec_ins = dynamic_cast<VanadisSpeculatedInstruction*>(next_ins);
                    bundle_has_branch = true;
                    auto [ predicted, predicted_address ] = branch_predictor->predictAddressIfAvailable(ip);

                    if ( predicted ) {
                        // We have an address predicton from the branching unit
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(CALL_INFO, 16, 0,
                            "%" PRIu32 ": ----> contains a branch: 0x%" PRI_ADDR " / predicted (found in predictor): 0x%" PRI_ADDR "\n",
                            hw_thr, ip, predicted_address);
                        #endif
                        ip = predicted_address;
                    } else {
                        // We don't have an address prediction so just speculate that we are going to drop through to the
                        // next instruction as we aren't sure where this will go yet
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(CALL_INFO, 16, 0,
                            "%" PRIu32 ": ----> contains a branch: 0x%" PRI_ADDR " / predicted "
                            "(not-found in predictor): 0x%" PRI_ADDR ", pc-increment: %" PRIu64 "\n", hw_thr, ip, ip + 4, bundle->pcIncrement());
                        #endif
                        ip += bundle->pcIncrement();
                    }
                    // Set speculated address
                    next_spec_ins->setSpeculatedAddress(ip);
                }

                auto new_ins = next_ins->clone();
                thread_rob->push(new_ins);
            }

            // Move to the next address, if we had a branch we should have already found a predicted target addeess to decode
            #ifdef VANADIS_BUILD_DEBUG
            if(output_->getVerboseLevel() >= 16) {
                output_->verbose(CALL_INFO, 16,0, "%" PRIu32 ": ----> branch? %s, ip=0x%" PRI_ADDR " + inc=%" PRIu64 " = new-ip=0x%" PRI_ADDR "\n",
                    hw_thr, bundle_has_branch ? "yes" : "no", ip, bundle_has_branch ? 0 : bundle->pcIncrement(),
                    bundle_has_branch ? ip : ip + bundle->pcIncrement());
            }
            #endif

            ip = bundle_has_branch ? ip : ip + bundle->pcIncrement();
            success = true;
        }
        else {
            #ifdef VANADIS_BUILD_DEBUG
            if (output_->getVerboseLevel() >= 16) {
                output_->verbose(CALL_INFO, 16, 0,
                    "%" PRIu32 ": ----> Attempt decode ip=0x%" PRI_ADDR ": uop cache hit but not enough space in the ROB for %" PRIu32 " entries. Stalling this cycle.\n",
                    hw_thr, ip, bundle->getInstructionCount());
                }
            #endif
            stat_uop_delayed_rob_full_->addData(1);
        }
    }
    else if ( ins_loader->hasPredecodeAt(ip, 4) ) {
        // We have a loaded instruction cache line but have not decoded it yet
        #ifdef VANADIS_BUILD_DEBUG
        if(output_->getVerboseLevel() >= 16) {
            output_->verbose(CALL_INFO, 16, 0,
                "%" PRIu32 ": ---> uop not found, but is located in the predecode i0-icache (ip=0x%" PRI_ADDR ")\n", hw_thr, ip);
        }
        #endif

        VanadisInstructionBundle* decoded_bundle = new VanadisInstructionBundle(ip);
        stat_predecode_hit_->addData(1);

        uint32_t temp_ins = 0;

        const bool predecode_bytes =
            ins_loader->getPredecodeBytes(ip, (uint8_t*)&temp_ins, sizeof(temp_ins));

        if ( LIKELY(predecode_bytes) ) {
            decode(ip, temp_ins, decoded_bundle);

            #ifdef VANADIS_BUILD_DEBUG
            if(output_->getVerboseLevel() >= 16) {
                output_->verbose(CALL_INFO, 16, 0,
                    "---> bundle generates %" PRIu32 " micro-ops\n", (uint32_t)decoded_bundle->getInstructionCount());
            }
            #endif

            ins_loader->cacheDecodedBundle(decoded_bundle);

            if ( 0 == decoded_bundle->getInstructionCount() ) {
                output_->fatal(CALL_INFO, -1, "Error - bundle at: 0x%" PRI_ADDR " generates no micro-ops.\n", ip);
            }
        }
        else {
            output_->fatal(CALL_INFO, -1,
                "Error - predecoded bytes for 0x%" PRIu64 " found, but retrieval of bytes failed.\n", ip);
        }
        success = true;
    }
    else if (!ins_loader->pendingLoad(ip, 4)) {
        // Not in micro or predecode cache, so we have to regenerate a request and stop further processing
        #ifdef VANADIS_BUILD_DEBUG
        if(output_->getVerboseLevel() >= 16) {
            output_->verbose(CALL_INFO, 16, 0,
                "%" PRIu32 " ---> microop bundle and pre-decoded bytes are not found for 0x%" PRI_ADDR ", requested read for cache line (line=%" PRIu64 ")\n",
                hw_thr, ip, ins_loader->getCacheLineWidth());
        }
        #endif
        ins_loader->requestLoadAt(ip, 4);
        stat_ins_bytes_loaded_->addData(4);
        stat_predecode_miss_->addData(1);
        success = true;
    }

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, 0, "%" PRIu32 " ---> decode execute completed, ip=0x%" PRI_ADDR "\n", hw_thr, ip);
    #endif
    return success;
}

void VanadisRISCV64Decoder::decode(const uint64_t ins_address, const uint32_t ins, VanadisInstructionBundle* bundle)
{
    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, 0, "%" PRIu32 ": ---> DECODE addr: 0x%" PRI_ADDR " / ins: 0x%08x\n", hw_thr, ins_address, ins);
    #endif

    // We are supposed to have 16b packets for RISCV instructions, if we don't then mark fault
    if ( (ins_address & 0x1) != 0 ) {
        bundle->addInstruction(new VanadisInstructionDecodeAlignmentFault(ins_address, hw_thr, options_));
        return;
    }

    uint32_t op_code = extract_opcode(ins);

    bool decode_fault = true;

    // if the last two bits that are set are 11, then we are performing at least 32bit instruction formats,
    // otherwise we are performing decodes on the C-extension (16b) formats
    if ( (ins & 0x3) == 0x3 ) {
        decode_fault = decodeBaseISA(op_code, ins_address, ins, bundle);
    } else {
        decode_fault = decodeCompressedExtension(op_code, ins_address, ins, bundle);
    }

    if ( decode_fault ) {

        if ( fatal_decode_fault_ ) {
            output_->fatal(CALL_INFO, -1,
                "[decode] -> decode fault detected at 0x%" PRI_ADDR " / thr: %" PRIu32 ", set to fatal on detect\n", ins_address, hw_thr);
        }
        bundle->addInstruction(new VanadisInstructionDecodeFault(ins_address, hw_thr, options_));
    }
}

bool
VanadisRISCV64Decoder::decodeBaseISA(const uint32_t op_code, const uint64_t ins_address, const uint32_t ins, VanadisInstructionBundle* bundle)
{
    uint16_t rd  = 0;
    uint16_t rs1 = 0;
    uint16_t rs2 = 0;
    uint16_t rs3 = 0;

    uint32_t func_code3 = 0;
    uint32_t func_code7 = 0;
    uint64_t uimm64     = 0;
    int64_t  simm64     = 0;

    switch ( op_code ) {

    case RVOP_LOAD: // I-type
    {
        // Load data
        processI<int64_t>(ins, rd, rs1, func_code3, simm64);

        switch ( func_code3 ) {
        case 0: // LB
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LB %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif

            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 1, true, MEM_TRANSACTION_NONE, LOAD_INT_REGISTER));
            return false;
        } break;
        case 1: // LH
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LH %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 2, true, MEM_TRANSACTION_NONE, LOAD_INT_REGISTER));
            return false;
        } break;
        case 2: // LW
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LW %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 4, true, MEM_TRANSACTION_NONE, LOAD_INT_REGISTER));
            return false;
        } break;
        case 3: // LD
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LD %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 8, true, MEM_TRANSACTION_NONE, LOAD_INT_REGISTER));
            return false;
        } break;
        case 4: // LBU
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "          LBU %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 1, false, MEM_TRANSACTION_NONE,
                LOAD_INT_REGISTER));
            return false;
        } break;
        case 5: // LHU
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LHU %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 2, false, MEM_TRANSACTION_NONE, LOAD_INT_REGISTER));
            return false;
        } break;
        case 6: // LWU
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "          LWU %" PRIu16 " <- %" PRIu16 " %" PRId64 "\n", rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 4, false, MEM_TRANSACTION_NONE,
                LOAD_INT_REGISTER));
            return false;
        } break;
        default: // Unknown load
            return true;
        }
    } break;

    case RVOP_LOAD_FP: // I-type
    {
        processI<int64_t>(ins, rd, rs1, func_code3, simm64);

        // Floating point load
        switch ( func_code3 ) {
        case 0x2: // FLW
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "%" PRIu32 ": ----> FLW %" PRIu16 " <- memory[ %" PRIu16 " + %" PRId64 " ]\n", hw_thr, rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 4, true, MEM_TRANSACTION_NONE, LOAD_FP_REGISTER));
            return false;
        } break;
        case 0x3:
        {
            // FLD
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "%" PRIu32 ": ----> FLD %" PRIu16 " <- memory[ %" PRIu16 " + %" PRId64 " ]\n", hw_thr, rd, rs1, simm64);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rd, 8, true, MEM_TRANSACTION_NONE, LOAD_FP_REGISTER));
            return false;
        } break;
        default:
            return true;
        }
    } break;

    case RVOP_CUSTOM0: // RoCC0
    {
        // custom encoding space for RoCC0 instructions
        // space derived from custom-0 in table 19.1 of RISC-V Manual
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0,
                "----> RoCC w/ accelerator id: %" PRIu8 ", rd: %" PRIu16 ", rs1: %" PRIu16
                ", rs2: %" PRIu16 ", and func7: %" PRIu32 "\n", 0, rd, rs1, rs2, func_code7);
        #endif
        bundle->addInstruction(new VanadisRoCCInstruction(
            ins_address, hw_thr, options_, rs1, rs2, rd, func_code3 & 0x1, func_code3 & 0x2, func_code3 & 0x4, func_code7, 0));
        return false;
    } break;

    case RVOP_STORE: // S-type
    {
        // Store data
        processS<int64_t>(ins, rs1, rs2, func_code3, simm64);

        if ( func_code3 < 4 ) { // SB, SH, SW, SD
            // shift to get the power of 2 number of bytes to store
            const uint32_t store_bytes = 1 << func_code3;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0,
                "----> STORE width: %" PRIu32 " bytes == (1 << %" PRIu32 ") %" PRIu16 " -> memory[ %" PRIu16
                " + %" PRId64 " / (0x%" PRI_ADDR ")]\n", store_bytes, func_code3, rs2, rs1, simm64, simm64);
            #endif
            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, rs1, simm64, rs2, store_bytes, MEM_TRANSACTION_NONE, STORE_INT_REGISTER));
            return false;
        }
        return true;
    } break;

    case RVOP_CUSTOM1: // RoCC1
    {
        // custom encoding space for RoCC1 instructions
        // space derived from custom-1 in table 19.1 of RISC-V Manual
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(
                CALL_INFO, 16, 0, "----> RoCC w/ accelerator id: %" PRIu8 ", rd: %" PRIu16 ", rs1: %" PRIu16
                ", rs2: %" PRIu16 ", and func7: %" PRIu32 "\n",
                1, rd, rs1, rs2, func_code7);
        #endif
        bundle->addInstruction(new VanadisRoCCInstruction(
            ins_address, hw_thr, options_, rs1, rs2, rd, func_code3 & 0x1, func_code3 & 0x2,
            func_code3 & 0x4, func_code7, 1));
        return false;
    } break;

    case RVOP_OP_IMM: // I-type, some specialized
    {
        // Immediate arithmetic
        processIbase(ins, rd, rs1, func_code3);

        switch ( func_code3 ) {
        case 0x0: // ADDI
        {
            processIimm<int64_t>(ins, simm64);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "------> ADDI %" PRIu16 " <- %" PRIu16 " + %" PRId64 "\n", rd, rs1, simm64);
            #endif

            bundle->addInstruction(new VanadisAddImmInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        case 0x1: // SLLI
        {
            uint32_t shamt;
            processIshamt(ins, shamt, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "------> SLLI / shift = %" PRIu32 " (0x%" PRIx32 ")\n", shamt, shamt);
            #endif

            switch ( func_code7 ) {
            case 0x0:
            {
                bundle->addInstruction(new VanadisShiftLeftLogicalImmInstruction<uint64_t>(ins_address, hw_thr, options_, rd, rs1, shamt));
                return false;
            } break;
            default:
                return true;
            };
        } break;
        case 0x2: // SLTI
        {
            processIimm<int64_t>(ins, simm64);
            bundle->addInstruction(new VanadisSetRegCompareImmInstruction<REG_COMPARE_LT, int64_t>(ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        case 0x3: // SLTIU
        {
            processIimm<int64_t>(ins, simm64);
            bundle->addInstruction(new VanadisSetRegCompareImmInstruction<REG_COMPARE_LT, uint64_t>(
                ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        case 0x4: // XORI
        {
            processIimm<int64_t>(ins, simm64);
            bundle->addInstruction(new VanadisXorImmInstruction(ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        case 0x5: // SRLI / SRAI
        {
            // bit 10 of immediate encodes R shift type (bit 30 of ins)
            uint32_t shamt;
            processIshamt(ins, shamt, func_code7);

            if ( func_code7 ) { // SRAI
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "--------> SRAI %" PRIu16 " <- %" PRIu16 " >> %" PRIu32 "\n", rd, rs1, shamt);
                #endif
                bundle->addInstruction(
                    new VanadisShiftRightArithmeticImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rd, rs1, shamt));
            } else { // SRLI
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "--------> SRLI %" PRIu16 " <- %" PRIu16 " >> %" PRIu32 "\n", rd, rs1, shamt);
                #endif
                bundle->addInstruction(
                    new VanadisShiftRightLogicalImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rd, rs1, shamt));
            }
            return false;
        } break;
        case 0x6: // ORI
        {
            processIimm<int64_t>(ins, simm64);
            bundle->addInstruction(new VanadisOrImmInstruction(ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        case 0x7: // ANDI
        {
            processIimm<int64_t>(ins, simm64);
            bundle->addInstruction(new VanadisAndImmInstruction(ins_address, hw_thr, options_, rd, rs1, simm64));
            return false;
        } break;
        };
        return true;
    } break;

    case RVOP_OP: // R-type
    {
        // integer arithmetic/logical
        processR(ins, rd, rs1, rs2, func_code3, func_code7);

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "-----> decode R-type, func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3, func_code7);
        #endif

        // Flatten func_code into <func_code7, func_code3>
        uint32_t func_code = (func_code7 << 3) | func_code3;

        switch ( func_code )
        {
            case 0x0: // ADD (0, 0)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> ADD %" PRIu16 " <- %" PRIu16 " + %" PRIu16 " hw_thr=%d\n", rd, rs1, rs2, hw_thr);
                #endif

                bundle->addInstruction(new VanadisAddInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x8: // MUL (1, 0)
            {
                // TODO - check register ordering
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> MUL %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisMultiplyInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x100:  // SUB (20, 0)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SUB %" PRIu16 " <- %" PRIu16 " - %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisSubInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2, false));
                return false;
            } break;
            case 0x1: // SLL (0, 1)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SLL %" PRIu16 " <- %" PRIu16 " << %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisShiftLeftLogicalInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x9: // MULH (1, 1)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> MULH %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(
                    new VanadisMultiplyHighInstruction<int64_t,int64_t>( ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x2: // SLT (0, 2)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SLT %" PRIu16 " <-  %" PRIu16 " < %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisSetRegCompareInstruction<VanadisRegisterCompareType::REG_COMPARE_LT, int64_t>(
                            ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xa:  // MULHSU (1, 2)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> MULHSU %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisMultiplyHighInstruction<int64_t,uint64_t>( ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x3: // SLTU (0, 3)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SLTU %" PRIu16 " <-  %" PRIu16 " < %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisSetRegCompareInstruction<
                        VanadisRegisterCompareType::REG_COMPARE_LT, uint64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xb:  // MULHU (1, 3)
            {
                // mul and place in upper XLEN bits, unsigned
                // need to check the register order
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> MULHU %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisMultiplyHighInstruction<uint64_t,uint64_t>( ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x4:  // XOR (0, 4)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> XOR %" PRIu16 " <-  %" PRIu16 " ^ %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisXorInstruction(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xc:  // DIV (1, 4)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> DIV %" PRIu16 " <-  %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisDivideInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x5: // SRL (0, 5)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SRL %" PRIu16 " <-  %" PRIu16 " >> %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisShiftRightLogicalInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xd: // DIVU (1, 5)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> DIVU %" PRIu16 " <-  %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisDivideInstruction<uint64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x105: // SRA (32, 5)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> SRA %" PRIu16 " <- %" PRIu16 " >> %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisShiftRightArithmeticInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x6: // OR (0, 6)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> OR %" PRIu16 " <- %" PRIu16 " | %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisOrInstruction(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xe: // REM (1, 6)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> REM %" PRIu16 " <- %" PRIu16 " %% %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisModuloInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x7:  // AND (0, 7)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> AND %" PRIu16 " <- %" PRIu16 " & %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisAndInstruction(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0xf:  // REMU (1, 7)
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> REMU %" PRIu16 " <- %" PRIu16 " %% %" PRIu16 "\n", rd, rs1, rs2);
                #endif

                bundle->addInstruction(new VanadisModuloInstruction<uint64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            default:
            {
                return true;
            }
        }
    } break;

    case RVOP_LUI: // U-type
    {
        int32_t uimm32 = 0;
        processU<int32_t>(ins, rd, uimm32);;

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "----> LUI %" PRIu16 " <- %#" PRIx32 "\n", rd, uimm32);
        #endif
        bundle->addInstruction(new VanadisSetRegisterInstruction<int32_t>(ins_address, hw_thr, options_, rd, uimm32));
        return false;
    } break;

    case RVOP_AUIPC: // U-type
    {
        processU<int64_t>(ins, rd, simm64);

        bundle->addInstruction(new VanadisPCAddImmInstruction<int64_t>(ins_address, hw_thr, options_, rd, simm64));
        return false;
    } break;

    case RVOP_OP_IMM_32: // I-type
    {
        processIbase(ins, rd, rs1, func_code3);
        //processR(ins, rd, rs1, rs2, func_code3, func_code7);

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "-----> decode I-type, func_code3=%" PRIu32 "\n", func_code3);
        #endif

        switch ( func_code3 ) {
        case 0x0: // ADDIW
        {
            //int64_t addiw_imm = 0;
            int32_t simm32;
            processIimm<int32_t>(ins, simm32);
            //processI(ins, rd, rs1, func_code3, addiw_imm);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-------> ADDIW %" PRIu16 " <- %" PRIu16 " + %" PRId32 "\n", rd, rs1,
                simm32);
            #endif

            bundle->addInstruction(new VanadisAddImmInstruction<int32_t>(ins_address, hw_thr, options_, rd, rs1, simm32));
            return false;
        } break;
        case 0x1: // SLLIW
        {
            uint32_t shamt;
            processIshamt(ins, shamt, func_code7);
            switch ( func_code7 ) {
            case 0x0:
            {
                // RS2 acts as an immediate
                // SLLIW (32bit result generated)
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-------> SLLIW %" PRIu16 " <- %" PRIu16 " << %" PRIu16 " (%#" PRIx16 ")\n", rd,
                    rs1, shamt, shamt);
                #endif
                bundle->addInstruction(
                    new VanadisShiftLeftLogicalImmInstruction<uint32_t>(
                        ins_address, hw_thr, options_, rd, rs1, shamt));
                return false;
            } break;
            }
        } break;
        case 0x5: // SRLIW, SRAIW
        {
            uint32_t shamt;
            processIshamt(ins, shamt, func_code7);
            switch ( func_code7 ) {
            case 0x0: // SRLIW
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-------> SRLIW %" PRIu16 " <- %" PRIu16 " << %" PRIu16 " (%#" PRIx16 ")\n", rd, rs1, shamt, shamt);
                #endif
                bundle->addInstruction(new VanadisShiftRightLogicalImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT32>(
                        ins_address, hw_thr, options_, rd, rs1, shamt));
                return false;

            } break;
            case 0x10: // SRAIW
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-------> SRAIW %" PRIu16 " <- %" PRIu16 " << %" PRIu16 " (%#" PRIx16 ")\n", rd, rs1, shamt, shamt);
                #endif
                bundle->addInstruction(new VanadisShiftRightArithmeticImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT32>(
                        ins_address, hw_thr, options_, rd, rs1, shamt));
                return false;
            } break;
            }
        } break;
        }
    } break;
    case RVOP_JAL: // J-type
    {
        processJ<int64_t>(ins, rd, simm64);
        // Immediate specifies jump in multiples of 2 byts per RISCV spec
        const int64_t jump_to = static_cast<int64_t>(ins_address) + simm64;

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0,
            "-----> JAL link-reg: %" PRIu16 " / jump-address: 0x%" PRI_ADDR " + %" PRId64 " = 0x%" PRI_ADDR "\n", rd, ins_address,
            simm64, jump_to);
        #endif

            bundle->addInstruction(new VanadisJumpLinkInstruction(
                ins_address, hw_thr, options_, 4, rd, jump_to, VANADIS_NO_DELAY_SLOT));
        return false;
    } break;

    case RVOP_JALR: // J-type
    {
        processI<int64_t>(ins, rd, rs1, func_code3, simm64);

        switch ( func_code3 ) {
        case 0:
        {
            // TODO - may need to zero bit 1 with an AND microop?

            bundle->addInstruction(new VanadisJumpRegLinkInstruction(
                ins_address, hw_thr, options_, 4, rd, rs1, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        };
    } break;

    case RVOP_BRANCH: // B-type
    {
        // Branch
        processB<int64_t>(ins, rs1, rs2, func_code3, simm64);

        switch ( func_code3 ) {
        case 0: // BEQ
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> BEQ %" PRIu16 " == %" PRIu16 " / offset: %" PRId64 "\n", rs1, rs2, simm64);
            #endif
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<int64_t, REG_COMPARE_EQ>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 1: // BNE
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0,
                "-----> BNE %" PRIu16 " != %" PRIu16 " / offset: %" PRId64 " (ip+offset %" PRIu64 ")\n", rs1,
                rs2, simm64, ins_address + simm64);
            #endif
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<int64_t, REG_COMPARE_NEQ>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 4: // BLT
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> BLT %" PRIu16 " < %" PRIu16 " / offset: %" PRId64 "\n", rs1, rs2, simm64);
            #endif
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<int64_t, REG_COMPARE_LT>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 5: // BGE
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> BGE %" PRIu16 " >= %" PRIu16 " / offset: %" PRId64 "\n", rs1, rs2, simm64);
            #endif
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<int64_t, REG_COMPARE_GTE>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 6: // BLTU
        {
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<uint64_t, REG_COMPARE_LT>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 7: // BGEU
        {
            bundle->addInstruction(new VanadisBranchRegCompareInstruction<uint64_t, REG_COMPARE_GTE>(
                ins_address, hw_thr, options_, 4, rs1, rs2, simm64, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        };
        return true;
    } break;

    case RVOP_SYSTEM:
    {
        // Syscall/ECALL and EBREAK
        // Control registers

        processI<uint64_t>(ins, rd, rs1, func_code3, uimm64);

        if ( (0 == rd) && (0 == rs1) && (0 == func_code3) ) {
            uint32_t func_code12 = (ins & 0xFFF00000);

            switch ( func_code12 ) {
            case 0x0: // SCALL
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "------> ECALL/SYSCALL\n");
                #endif
                bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                bundle->addInstruction(new VanadisSysCallInstruction(ins_address, hw_thr, options_));
                return false;
            } break;
            case 0x1: // SBREAK
            {
                printf("%#" PRIx64 " %#x SBREAK\n",ins_address,ins);
                assert(0);
            } break;
            }
        } else {
            // CSRRW(I) atomic reaad/write
            // CSRRS(I) atomic read and set bits
            // CSRRC(I) atomic read and clear bit

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                    CALL_INFO, 16, 0, "-----> %s: ins: 0x%" PRI_ADDR " / %#" PRIx64 " / rd: %" PRIu16 " / rs1: %" PRIu16 "\n",
                                getCSR_name(func_code3).c_str(),ins_address, uimm64, rd, rs1);
            #endif

            switch ( uimm64 )
            {
                case 0x1: // FSFLAGS
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose( CALL_INFO, 16, 0, "----->  FFLAGS: %" PRIu64 " / rd: %" PRIu16 " / rs1: %" PRIu16 "\n", uimm64, rd, rs1);
                    #endif

                    if ( ! ( ( 0x1 == func_code3 || 0x3 == func_code3 ) && 0 == rd ) ) {

                        bundle->addInstruction(new VanadisFPFlagsReadInstruction<false, false, true>(ins_address, hw_thr, options_, fpflags, rd));
                    }
                    if ( ! ( 0x2 == func_code3 && rs1 == 0 ) ) {
                        if ( func_code3 & 0x4 ) {
                            bundle->addInstruction(
                                new VanadisFPFlagsSetImmInstruction<false,true>(ins_address, hw_thr, options_, fpflags, static_cast<uint64_t>(rs1), func_code3 & 0x3));
                        } else {
                            bundle->addInstruction(
                                new VanadisFPFlagsSetInstruction<false,true>(ins_address, hw_thr, options_, fpflags, rs1, func_code3 & 0x3 ));
                        }
                    }
                    return false;
                } break;
                case 0x2: // FSRM
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose( CALL_INFO, 16, 0, "----->  FRM: %" PRIu64 " / rd: %" PRIu16 " / rs1: %" PRIu16 "\n", uimm64, rd, rs1);
                    #endif

                    if ( ! ( ( 0x1 == func_code3 || 0x3 == func_code3 ) && 0 == rd ) ) {

                        bundle->addInstruction(new VanadisFPFlagsReadInstruction<true, false, false>(ins_address, hw_thr, options_, fpflags, rd));
                    }
                    if ( ! ( 0x2 == func_code3 && rs1 == 0 ) ) {
                            if ( func_code3 & 0x4 ) {
                            bundle->addInstruction(new VanadisFPFlagsSetImmInstruction<true,false>(
                                ins_address, hw_thr, options_, fpflags, static_cast<uint64_t>(rs1), func_code3 & 0x3));
                        } else {

                            bundle->addInstruction(
                                new VanadisFPFlagsSetInstruction<true,false>(ins_address, hw_thr, options_, fpflags, rs1, func_code3 & 0x3));
                        }
                    }
                    return false;
                } break;
                case 0x3: // FCSR
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose( CALL_INFO, 16, 0, "-----> FCSR: %" PRIu64 " / rd: %" PRIu16 " / rs1: %" PRIu16 "\n", uimm64, rd, rs1);
                    #endif

                    if ( ! ( ( 0x1 == func_code3 || 0x3 == func_code3 ) && 0 == rd ) ) {

                        bundle->addInstruction(new VanadisFPFlagsReadInstruction<true, true, true>(ins_address, hw_thr, options_, fpflags, rd));
                    }
                    if ( ! ( 0x2 == func_code3 && rs1 == 0 ) ) {
                        if ( func_code3 & 0x4 ) {
                            bundle->addInstruction(new VanadisFPFlagsSetImmInstruction<true,true>(
                                ins_address, hw_thr, options_, fpflags, static_cast<uint64_t>(rs1), func_code3 & 0x3));
                        } else {

                            bundle->addInstruction(
                                new VanadisFPFlagsSetInstruction<true,true>(ins_address, hw_thr, options_, fpflags, rs1, func_code3 & 0x3));
                        }
                    }
                    return false;
                } break;
                default:
                {
                uint64_t csrNum = uimm64 & 0xfff;
                switch ( csrNum ) {
                    case 0xc00:
                    {
                        if ( 0 == rs1 ) {
                            auto thread_call = std::bind(&VanadisRISCV64Decoder::getCycleCount, this);
                            bundle->addInstruction( new VanadisSetRegisterByCallInstruction<int64_t>(
                                ins_address, hw_thr, options_, rd, thread_call));
                            return false;
                        }
                    } break;
                }

                } break;
            }
        }
        return true;
    } break; // end of RVOP_SYSTEM:

    case RVOP_CUSTOM3:
    {
        // custom encoding space for RoCC3 instructions
        // space derived from custom-3 in table 19.1 of RISC-V Manual
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(
                CALL_INFO, 16, 0, "----> RoCC w/ accelerator id: %" PRIu8 ", rd: %" PRIu16 ", rs1: %" PRIu16
                ", rs2: %" PRIu16 ", and func7: %" PRIu32 "\n", 3, rd, rs1, rs2, func_code7);
        #endif
        bundle->addInstruction(new VanadisRoCCInstruction(
            ins_address, hw_thr, options_, rs1, rs2, rd, func_code3 & 0x1, func_code3 & 0x2, func_code3 & 0x4, func_code7, 3));
        return false;
    } break;

    case RVOP_OP_32:
    {
        // 64b integer arithmetic-W
        processR(ins, rd, rs1, rs2, func_code3, func_code7);

        switch ( func_code3 ) {
        case 0:
        {
            switch ( func_code7 ) {
            case 0x0: // ADDW
            {
                // TODO - check register ordering
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-------> ADD %" PRIu16 " <- %" PRIu16 " + %" PRIu16 " hw_thr=%d\n", rd, rs1, rs2, hw_thr);
                #endif

                bundle->addInstruction(
                    new VanadisAddInstruction<int64_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x1: // MULW
            {
                // TODO - check register ordering
                bundle->addInstruction(new VanadisMultiplyInstruction<int32_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x20: // SUBW
            {
                // TODO - check register ordering
                bundle->addInstruction(new VanadisSubInstruction<int32_t>(ins_address, hw_thr, options_, rd, rs1, rs2, true));
                return false;
            } break;
            };
            return true;
        } break;
        case 1:
        {
            if ( func_code7 != 0x0 ) { return true; }

            // SLLW
            // TODO - check register ordering
            bundle->addInstruction(new VanadisShiftLeftLogicalInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT32>(
                    ins_address, hw_thr, options_, rd, rs1, rs2));
            return false;
        } break;
        case 4:
        {
            if ( func_code7 != 0x1 ) { return true; }

            // DIVW
            bundle->addInstruction(new VanadisDivideInstruction<int32_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
            return false;
        } break;
        case 5:
        {
            switch ( func_code7 ) {
            case 0x0: // SRLW
            {
                // TODO - check register ordering
                bundle->addInstruction(
                    new VanadisShiftRightLogicalInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT32>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x1: // DIVUW
            {
                bundle->addInstruction(
                    new VanadisDivideInstruction<uint32_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            case 0x20: // SRAW
            {
                // TODO - check register ordering
                bundle->addInstruction(
                    new VanadisShiftRightArithmeticInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT32>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            };
            return true;
        } break;
        case 6:
        {
            switch ( func_code7 ) {
            case 0x1:
            {
                // REMW
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "----> REMW %" PRIu16 " <- %" PRIu16 " %% %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(
                    new VanadisModuloInstruction<int32_t>(
                        ins_address, hw_thr, options_, rd, rs1, rs2));
                return false;
            } break;
            };
        } break;
        case 7:
        {
            if ( func_code7 != 0x1 ) { return true; }

            // REMUW
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "----> REMUW %" PRIu16 " <- %" PRIu16 " %% %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(new VanadisModuloInstruction<uint32_t>(ins_address, hw_thr, options_, rd, rs1, rs2));
            return false;
        }; break;
        };
        return true;
    } break;

    case RVOP_MISC_MEM:
    {
        func_code3 = extract_func3(ins);
        if ( func_code3 != 0x0 ) { return true; }

        // Fence operations
        // For now, we conduct a heavy fence. Could optimize this to be more efficient.
        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "----> FENCE\n");
        #endif
        bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
        return false;
    } break;

    case RVOP_AMO:
    {
        // Atomic operations (A extension)
        processR(ins, rd, rs1, rs2, func_code3, func_code7);

        const bool perform_aq = func_code7 & 0x2;
        const bool perform_rl = func_code7 & 0x1;

        uint32_t op_width = 0;

        switch(func_code3) {
        case 0x2:
            op_width = 4;
            break;
        case 0x3:
            op_width = 8;
            break;
        default:
            op_width = 0;
            break;
        }

        // need to mask out AQ and RL bits, these are used for deciding on fence operations
        auto amo_op = (func_code7 & 0x7C) >> 2;

        switch ( amo_op ) {
        case AMO_SWAP:
        {
            if(LIKELY(op_width != 0)) {
                // AMO.SWAP
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0,
                    "-----> AMOSWAP 0x%" PRI_ADDR " / thr: %" PRIu32 " / %" PRIu16 " <- memory[ %" PRIu16 " ] <- %" PRIu16
                    " / width: %" PRIu32 " / aq: %s / rl: %s\n",
                    ins_address, hw_thr, rd, rs1, rs2, op_width, perform_aq ?  "yes" : "no", perform_rl ? "yes" : "no");
                #endif
                if(LIKELY(perform_aq)) {

                    bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                }

                // implement a micro-op sequence for AMOSWAP (yuck)
                bundle->addInstruction(new VanadisLoadInstruction(
                            ins_address, hw_thr, options_, rs1, 0, /* place in r34 */ 34, op_width,
                            true, MEM_TRANSACTION_LLSC_LOAD, LOAD_INT_REGISTER));

                // 0 is a success, 1 is a failure
                bundle->addInstruction(new VanadisStoreConditionalInstruction(
                        ins_address, hw_thr, options_, rs1, 0, rs2, /* place in r33 */ 33, op_width,
                        STORE_INT_REGISTER, 0, 1));

                // conditionally copy the loaded value into rd IF the store-conditional was successful
                // copy r32 into rd if r33 == 0

                bundle->addInstruction(new VanadisConditionalMoveImmInstruction<int64_t, int64_t, REG_COMPARE_EQ>(
                        ins_address, hw_thr, options_, rd, 34, 33, 0
                ));


                // branch back to this instruction address IF r33 == 1 (which means the store failed)
                bundle->addInstruction(new VanadisBranchRegCompareImmInstruction<int64_t, REG_COMPARE_EQ>(
                    ins_address, hw_thr, options_, 4, 33, 1,
                    /* offset is 0 so we replay this instruction */ 0, VANADIS_NO_DELAY_SLOT
                ));

                if(LIKELY(perform_rl)) {
                    bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                }

                return false;
            }
        } break;
        case AMO_ADD:
        case AMO_XOR:
        case AMO_OR:
        case AMO_AND:
        case AMO_MIN:
        case AMO_MAX:
        case AMO_MINU:
        case AMO_MAXU:
        {
            if (UNLIKELY(op_width == 0)) { return true; }

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0,
                "-----> %s.%s 0x%" PRIx64 " / thr: %" PRIu32 " / %" PRIu16 " <- memory[ %" PRIu16 " ] <- %" PRIu16
                " / width: %" PRIu32 " / aq: %s / rl: %s\n",
                getAMO_name(amo_op).c_str(), getAMO_type( func_code3 ).c_str(),
                ins_address, hw_thr, rd, rs1, rs2, op_width, perform_aq ?  "yes" : "no", perform_rl ? "yes" : "no");
            #endif
            if (LIKELY(perform_aq)) {
                bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
            }

            // (r1) -> rd
            bundle->addInstruction(new VanadisLoadInstruction(
                        ins_address, hw_thr, options_, rs1, 0, 34, op_width, true, MEM_TRANSACTION_LLSC_LOAD, LOAD_INT_REGISTER));

            // r34 OP rs2 => r32
            switch( amo_op ) {
            case AMO_ADD:
                bundle->addInstruction( new VanadisAddInstruction<int64_t>( ins_address, hw_thr, options_, 32, 34, rs2));
                break;
            case AMO_XOR:
                bundle->addInstruction( new VanadisXorInstruction( ins_address, hw_thr, options_, 32, 34, rs2));
                break;
            case AMO_OR:
                bundle->addInstruction( new VanadisOrInstruction( ins_address, hw_thr, options_, 32, 34, rs2));
                break;
            case AMO_AND:
                bundle->addInstruction( new VanadisAndInstruction( ins_address, hw_thr, options_, 32, 34, rs2));
                break;
            case AMO_MIN:
                if ( AMO_W == func_code3 ) {
                    bundle->addInstruction( new VanadisMinInstruction<int32_t,true>( ins_address, hw_thr, options_, 32, 34, rs2));
                } else {
                    bundle->addInstruction( new VanadisMinInstruction<int64_t,true>( ins_address, hw_thr, options_, 32, 34, rs2));
                }
                break;
            case AMO_MAX:
                if ( AMO_W == func_code3 ) {
                    bundle->addInstruction( new VanadisMinInstruction<int32_t,false>( ins_address, hw_thr, options_, 32, 34, rs2));
                } else {
                    bundle->addInstruction( new VanadisMinInstruction<int64_t,false>( ins_address, hw_thr, options_, 32, 34, rs2));
                }
                break;
            case AMO_MINU:
                if ( AMO_W == func_code3 ) {
                    bundle->addInstruction( new VanadisMinInstruction<uint32_t,true>( ins_address, hw_thr, options_, 32, 34, rs2));
                } else {
                    bundle->addInstruction( new VanadisMinInstruction<uint64_t,true>( ins_address, hw_thr, options_, 32, 34, rs2));
                }
                break;
            case AMO_MAXU:
                if ( AMO_W == func_code3 ) {
                    bundle->addInstruction( new VanadisMinInstruction<uint32_t,false>( ins_address, hw_thr, options_, 32, 34, rs2));
                } else {
                    bundle->addInstruction( new VanadisMinInstruction<uint64_t,false>( ins_address, hw_thr, options_, 32, 34, rs2));
                }
                break;
            default: assert(0);
            }

            // r32 -> (rs1) and place success code in r33
            // 0 is a success, 1 is a failure
            bundle->addInstruction(new VanadisStoreConditionalInstruction(
                    ins_address, hw_thr, options_, rs1, 0, 32, /* success code */ 33, op_width,
                    STORE_INT_REGISTER, 0, 1));

            // conditionally copy the loaded value into rd IF the store-conditional was successful
            // copy r34 into rd if r33 == 0
            bundle->addInstruction(new VanadisConditionalMoveImmInstruction<int64_t, int64_t, REG_COMPARE_EQ>(
                    ins_address, hw_thr, options_, rd, 34, 33, 0));
            // branch back to this instruction address IF r33 == 1 (which means the store failed)
            bundle->addInstruction(new VanadisBranchRegCompareImmInstruction<int64_t, REG_COMPARE_EQ>(
                ins_address, hw_thr, options_, 4, 33, 1,
                /* offset is 0 so we replay this instruction */ 0, VANADIS_NO_DELAY_SLOT));

            if (LIKELY(perform_rl)) {
                bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
            }

            return false;
        } break;
        case LR:
        {
            if ( rs2 == 0 ) {
                // LR.?.AQ.RL
                if(LIKELY(op_width != 0)) {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "-----> LR 0x%" PRI_ADDR " / thr: %" PRIu32 " / (LLSC_LOAD) %" PRIu16 " <- memory[ %" PRIu16 " ] / width: %" PRIu32 " / aq: %s / rl: %s\n",
                            ins_address, hw_thr, rd, rs1, op_width, perform_aq ?  "yes" : "no", perform_rl ? "yes" : "no");
                    #endif

                    if(LIKELY(perform_aq)) {

                        bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                    }

                    bundle->addInstruction(new VanadisLoadInstruction(
                        ins_address, hw_thr, options_, rs1, 0, rd, op_width, true, MEM_TRANSACTION_LLSC_LOAD,
                        LOAD_INT_REGISTER));

                    if(LIKELY(perform_rl)) {

                        bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                    }

                    return false;
                }
            }
            else {
                // ?
            }
        } break;
        case SC:
        {
            if(LIKELY(op_width != 0)) {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0,
                    "-----> SC 0x%" PRI_ADDR " / thr: %" PRIu32 " / (LLSC_STORE) %" PRIu16 " -> memory[ %" PRIu16 " ] / result: %" PRIu16 " / width: %" PRIu32 " / aq: %s / rl: %s\n",
                    ins_address, hw_thr, rs2, rs1, rd, op_width, perform_aq ?  "yes" : "no", perform_rl ? "yes" : "no");
                #endif

                if(LIKELY(perform_aq)) {

                    bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                }

                bundle->addInstruction(new VanadisStoreConditionalInstruction(
                    ins_address, hw_thr, options_, rs1, 0, rs2, rd, op_width, STORE_INT_REGISTER, 0, 1));

                if(LIKELY(perform_rl)) {

                    bundle->addInstruction(new VanadisFenceInstruction(ins_address, hw_thr, options_, VANADIS_LOAD_STORE_FENCE));
                }

                return false;
            }
        } break;
        }
        return true;
    } break;

    case RVOP_STORE_FP:
    {
        // Floating point store
        processS<int64_t>(ins, rs1, rs2, func_code3, simm64);

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "---> STORE-FP func_code3=%" PRIu32 " imm=%" PRId64 " / rs1: %" PRIu16 " / rs2: %" PRIu16 "\n",
                func_code3, simm64, rs1, rs2);
        #endif

        switch(func_code3) {
            case 0x2:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> FSW imm=%" PRId64 " / rs1: %" PRIu16 " / rs2: %" PRIu16 "\n", simm64, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisStoreInstruction(
                    ins_address, hw_thr, options_, rs1, simm64, rs2, 4, MEM_TRANSACTION_NONE, STORE_FP_REGISTER));
                return false;
            } break;
            case 0x3:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-------> FSD imm=%" PRId64 " / rs1: %" PRIu16 " / rs2: %" PRIu16 "\n", simm64, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisStoreInstruction(
                    ins_address, hw_thr, options_, rs1, simm64, rs2, 8, MEM_TRANSACTION_NONE, STORE_FP_REGISTER));
                return false;
            } break;
        }
        return true;
    } break;

    case RVOP_OP_FP:
    {
        // floating point arithmetic
        processR(ins, rd, rs1, rs2, func_code3, func_code7);

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(
            CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3, func_code7);
        #endif
        switch ( func_code7 ) {
        case 44:
        {
            //FSQRT.S
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FSQRT.S %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
            #endif
            bundle->addInstruction(
                new VanadisFPSquareRootInstruction<float>(ins_address, hw_thr, options_, fpflags, rd, rs1));
            return false;
        } break;
        case 45:
        {
            //FSQRT.D
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FSQRT.D %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
            #endif
            bundle->addInstruction(
                new VanadisFPSquareRootInstruction<double>(ins_address, hw_thr, options_, fpflags, rd, rs1));
            return false;
        } break;
        case 0:
        {
            // FADD.S
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FADD.S %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPAddInstruction<float>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 0x1:
        {
            // FADD.D
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FADD.D %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPAddInstruction<double>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 4:
        {
            // FSUB.S
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FSUB.S %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPSubInstruction<float>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 5:
        {
            // FSUB.D
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FSUB.D %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPSubInstruction<double>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 8:
        {
            // FMUL.S
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FMUL.S %" PRIu16 " <- %" PRIu16 " * %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPMultiplyInstruction<float>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 9:
        {
            // FMUL.D
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> FMUL.D %" PRIu16 " <- %" PRIu16 " * %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(
                new VanadisFPMultiplyInstruction<double>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 16:
        {
            switch ( func_code3 ) {
            case 0:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJ.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<float, VanadisFPSignLogicOperation::SIGN_COPY>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 1:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJN.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<float, VanadisFPSignLogicOperation::SIGN_NEG>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 2:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJX.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<float, VanadisFPSignLogicOperation::SIGN_XOR>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            }
        } break;
        case 17:
        {
            switch ( func_code3 ) {
            case 0:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJ.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<double, VanadisFPSignLogicOperation::SIGN_COPY>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 1:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJN.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<double, VanadisFPSignLogicOperation::SIGN_NEG>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 2:
            {
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FSGNJX.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1,
                    rs2);
                #endif
                bundle->addInstruction(
                    new VanadisFPSignLogicInstruction<double, VanadisFPSignLogicOperation::SIGN_XOR>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            }
        } break;
        case 12:
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> FDIV.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(new VanadisFPDivideInstruction<float>(
                ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
                } break;
        case 0xD:
        {
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> FDIV.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
            #endif
            bundle->addInstruction(new VanadisFPDivideInstruction<double>(
                ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
            return false;
        } break;
        case 20:
        {
            switch ( func_code3 ) {
                case 0:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "---> FMIN.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                    #endif
                    bundle->addInstruction(new VanadisFPMinimumInstruction<float,true>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                    return false;
                } break;
                case 1:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "---> FMAX.S %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                    #endif
                    bundle->addInstruction(new VanadisFPMinimumInstruction<float,false>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                    return false;
                } break;
            }
        } break;
        case 21:
        {
            switch ( func_code3 ) {
                case 0:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "---> FMIN.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                    #endif
                    bundle->addInstruction(new VanadisFPMinimumInstruction<double,true>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                    return false;
                } break;
                case 1:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "---> FMAX.D %" PRIu16 " <- %" PRIu16 " / %" PRIu16 "\n", rd, rs1, rs2);
                    #endif
                    bundle->addInstruction(new VanadisFPMinimumInstruction<double,false>(
                        ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                    return false;
                } break;
            }
        } break;
        case 32:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose( CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3, func_code7);
            #endif
            switch ( rs2 ) {
                case 1:
                {
                    // double to float
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.S.D %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                    #endif
                    bundle->addInstruction(
                        new VanadisFPConvertInstruction<double,float>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                    return false;
                } break;
            }
        } break;
        case 33:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif
            // rs2 dictates the signed nature of the operands
                    // we use FP convert here because these are FP to FP regiser modifications
            switch ( rs2 ) {
            case 0:
            {
                // float to double
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.D.S %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFPConvertInstruction<float, double>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            }

        } break;
        case 104:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif

            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                // int32 to float
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.S.W %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<int32_t, float, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 1:
            {
                // uint32 to float
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.S.WU %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<uint32_t, float, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 2:
            {
                // int64 to float
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.S.L %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<int64_t, float, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 3:
            {
                // uint64 to float
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.S.LU %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<uint64_t, float, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            }
        } break;
        case 96:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif

            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                // float to int32
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.W.S %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<float, int32_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 1:
            {
                // float to uint32
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.WU.S %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<float, uint32_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 2:
            {
                // float to int64
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.L.S %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<float, int64_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 3:
            {
                // float to uint64
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.L.S %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<float, uint64_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            }
        } break;
        case 97:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);
            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif
            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                // double to int32
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.W.D %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<double, int32_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 1:
            {
                // double to uint32
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.WU.D %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<double, uint32_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 2:
            {
                // double to int64
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.L.D %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<double, int64_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            }break;
            case 3:
            {
                // double to uint64
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.LU.D %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisFP2GPRInstruction<double, uint64_t, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            }break;
            }
        } break;
        case 105:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif
            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                // int32 to double
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.D.W %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<int32_t, double, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 1:
            {
                // uint32 to double
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.D.WU %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<uint32_t, double, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 2:
            {
                // int64 to double
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.D.L %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<int64_t, double, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            case 3:
            {
                // uint64 to double
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> FCVT.D.LU %" PRIu16 " <- %" PRIu16 "\n", rs1, rd);
                #endif
                bundle->addInstruction(
                    new VanadisGPR2FPInstruction<uint64_t, double, false>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                return false;
            } break;
            }
        } break;
            case 0x70:
            {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif
            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                switch(func_code3) {
                    case 0: {
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(CALL_INFO, 16, 0, "-----> FMV.X.W %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                        #endif
                        bundle->addInstruction(
                        new VanadisFP2GPRInstruction<uint32_t, uint32_t, true>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                        return false;
                    } break;
                    case 1:
                    {
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(CALL_INFO, 16, 0, "-----> FCLASS.S %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                        #endif
                        bundle->addInstruction(
                        new VanadisFPClassInstruction<uint64_t,float>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                        return false;
                    } break;
                }
            } break;
            }
        } break;
            case 120:
            {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "---> func_code3=%" PRIu32 " / func_code7=%" PRIu32 "\n", func_code3,
                func_code7);
            #endif

            // rs2 dictates the signed nature of the operands
            switch ( rs2 ) {
            case 0:
            {
                switch(func_code3) {
                case 0:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "-----> FMV.W.X %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                    #endif

                    bundle->addInstruction(
                        new VanadisGPR2FPInstruction<uint32_t, uint32_t, true>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                    return false;
                } break;
                }
            } break;
            }
        } break;
        case 0x50:
        {
            switch ( func_code3 ) {
            case 0x0:
            {
                // FLE.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FLE.S %" PRIu16 " <- %" PRIu16 " <= %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_LTE, float>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 0x1:
            {
                // FLT.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FLT.S %" PRIu16 " <- %" PRIu16 " <= %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_LT, float>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 0x2:
            {
                // FEQ.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FEQ.S %" PRIu16 " <- %" PRIu16 " < %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_EQ, float>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            }
        } break;
        case 0x51:
        {
            switch ( func_code3 ) {
            case 0x0:
            {
                // FLE.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FLE.D %" PRIu16 " <- %" PRIu16 " <= %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_LTE, double>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 0x1:
            {
                // FLT.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FLT.D %" PRIu16 " <- %" PRIu16 " < %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_LT, double>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            case 0x2:
            {
                // FEQ.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FEQ.D %" PRIu16 " <- %" PRIu16 " < %" PRIu16 "\n", rd, rs1, rs2);
                #endif
                bundle->addInstruction(new VanadisFPSetRegCompareInstruction<REG_COMPARE_EQ, double>(
                    ins_address, hw_thr, options_, fpflags, rd, rs1, rs2));
                return false;
            } break;
            }
        } break;
        case 0x71:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            if ( 0 == rs2 ) {
                switch( func_code3 ) {
                case 0:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "-----> FMV.X.D %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                    #endif
                    bundle->addInstruction(
                        new VanadisFP2GPRInstruction<uint64_t, uint64_t, true>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                    return false;
                } break;
                case 1:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "-----> FCLASS.D %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                    #endif
                    bundle->addInstruction(
                        new VanadisFPClassInstruction<uint64_t,double>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                    return false;
                } break;
                }
            }
        } break;
        case 0x79:
        {
            processR(ins, rd, rs1, rs2, func_code3, func_code7);

            if ( 0 == rs2 ) {
                if ( 0 == func_code3 ) {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "-----> FMV.D.X %" PRIu16 " <- %" PRIu16 "\n", rd, rs1);
                    #endif
                    bundle->addInstruction(
                        new VanadisGPR2FPInstruction<uint64_t, uint64_t, true>(ins_address, hw_thr, options_, fpflags, rd, rs1));
                    return false;
                }
            }
        } break;
        }
        return true;
    } break;

    case RVOP_CUSTOM2:
    {
        // custom encoding space for RoCC2 instructions
        // space derived from custom-2 in table 19.1 of RISC-V Manual
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(
                CALL_INFO, 16, 0, "----> RoCC w/ accelerator id: %" PRIu8 ", rd: %" PRIu16 ", rs1: %" PRIu16
                ", rs2: %" PRIu16 ", and func7: %" PRIu32 "\n",
                2, rd, rs1, rs2, func_code7);
        #endif
        bundle->addInstruction(new VanadisRoCCInstruction(
            ins_address, hw_thr, options_, rs1, rs2, rd, func_code3 & 0x1, func_code3 & 0x2, func_code3 & 0x4, func_code7, 2));
        return false;
    } break;

    case RVOP_MADD:
    {
        uint32_t fmt;
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        fmt = func_code7 & 0x3;
        rs3 = func_code7 >> 2;

        switch( fmt ) {
            case 0:
            {
                // FMADD.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FMADD.S %" PRIu16 " <- ( %" PRIu16 " *  %" PRIu16 " ) + %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplyAddInstruction<float,false>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
            case 1:
            {
                // FMADD.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FMADD.D %" PRIu16 " <- ( %" PRIu16 " *  %" PRIu16 " ) + %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplyAddInstruction<double,false>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
        }
        return true;
    } break;

    case RVOP_MSUB:
    {
        // FMSUB
        uint32_t fmt;
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        fmt = func_code7 & 0x3;
        rs3 = func_code7 >> 2;

        switch( fmt ) {
            case 0:
            {
                // FMSUB.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FMSUB.S %" PRIu16 " <- ( %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplySubInstruction<float,false>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
            case 1:
            {
                // FMSUB.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FMSUB.D %" PRIu16 " <- ( %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplySubInstruction<double,false>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
        }
        return true;
    } break;

    case RVOP_NMSUB:
    {
        uint32_t fmt;
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        fmt = func_code7 & 0x3;
        rs3 = func_code7 >> 2;

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "-----> fmt: %" PRIu32 " rs3: %" PRIu32 "\n", fmt, rs3);
        #endif

        switch( fmt ) {
            case 0:
            {
                // FNMSUB.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FNMSUB.S %" PRIu16 " <- ( - %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplySubInstruction<float,true>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
            case 1:
            {
                // FNMSUB.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FNMSUB.D %" PRIu16 " <- ( - %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplySubInstruction<double,true>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
        }
        return true;
    } break;

    case RVOP_NMADD:
    {
        uint32_t fmt;
        processR(ins, rd, rs1, rs2, func_code3, func_code7);
        fmt = func_code7 & 0x3;
        rs3 = func_code7 >> 2;

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(CALL_INFO, 16, 0, "-----> fmt: %" PRIu32 " rs3: %" PRIu32 "\n", fmt, rs3);
        #endif

        switch( fmt ) {
            case 0:
            {
                // FNMADD.S
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FNMADD.S %" PRIu16 " <- ( - %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplyAddInstruction<float,true>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
            case 1:
            {
                // FNMADD.D
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> FNMADD.D %" PRIu16 " <- ( - %" PRIu16 " *  %" PRIu16 " ) -  %" PRIu16 "\n", rd, rs1, rs2, rs3);
                #endif
                bundle->addInstruction(
                    new VanadisFPFusedMultiplyAddInstruction<double,true>(ins_address, hw_thr, options_, fpflags, rd, rs1, rs2, rs3 ));
                return false;
            } break;
        }
        return true;
    } break;

    default:
    {
        // op_code did not match an expected value
        return true;
    } break;
    }

    return true; // Did not decode anything, return fault
}

bool
VanadisRISCV64Decoder::decodeCompressedExtension(const uint32_t op_code, const uint64_t ins_address, const uint32_t ins, VanadisInstructionBundle* bundle)
{
    uint16_t rd  = 0;
    uint16_t rs1 = 0;
    uint16_t rs2 = 0;
    uint16_t rs3 = 0;

    uint32_t func_code3 = 0;
    uint32_t func_code7 = 0;
    uint64_t uimm64     = 0;
    int64_t  simm64     = 0;

    const uint32_t c_op_code = ins & 0x3;

    // this bundle only increments the PC by 2, not 4 in 32bit decodes
    bundle->setPCIncrement(2);

    #ifdef VANADIS_BUILD_DEBUG
    output_->verbose(CALL_INFO, 16, 0, "-----> RVC op_code: %" PRIu32 "\n", c_op_code);
    #endif

    switch ( c_op_code ) {
    case 0x0:
    {
        const uint32_t c_func_code = ins & 0xE000;

        switch ( c_func_code ) {
        case 0x0:
        {
            if ( (ins & 0xFFFF) == 0 ) {
                // This is an illegal instruction (all zeroes)
            }
            else {
                // ADDI4SPN
                uint16_t rvc_rd = expand_rvc_int_register(extract_rs2_rvc(ins));

                uint32_t imm_3  = (ins & 0x20) >> 2;
                uint32_t imm_2  = (ins & 0x40) >> 4;
                uint32_t imm_96 = (ins & 0x780) >> 1;
                uint32_t imm_54 = (ins & 0x1800) >> 7;

                uint32_t imm = (imm_2 | imm_3 | imm_54 | imm_96);

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-------> RVC ADDI4SPN %" PRIu16 " <- r2 (stack-ptr) + %" PRIu32 "\n",
                    rvc_rd, imm);
                #endif

                bundle->addInstruction(
                    new VanadisAddImmInstruction<int64_t>(ins_address, hw_thr, options_, rvc_rd, 2, imm));
                return false;
            }
        } break;
        case 0x2000:
        {
            // FLD
            uint16_t rvc_rd  = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            uint32_t imm_53 = (ins & 0x1C00) >> 7;
            uint32_t imm_76 = (ins & 0x60) << 1;

            uint32_t imm = (imm_53 | imm_76);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-------> RVC FLD %" PRIu16 " <- memory[ %" PRIu16 " + %" PRIu32 " ]\n",
                rvc_rd, rvc_rs1, imm);
            #endif

            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rvc_rs1, imm, rvc_rd, 8, true, MEM_TRANSACTION_NONE,
                LOAD_FP_REGISTER));
            return false;
        } break;
        case 0x4000:
        {
            // LW
            uint64_t imm_address = extract_uimm_rcv_t2(ins);
            uint16_t rvc_rd      = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1     = expand_rvc_int_register(extract_rs1_rvc(ins));

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC LW rd=%" PRIu16 " <- rs1=%" PRIu16 " + imm=%" PRIu64 "\n", rvc_rd,
                rvc_rs1, imm_address);
            #endif

            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rvc_rs1, imm_address, rvc_rd, 4, true, MEM_TRANSACTION_NONE,
                LOAD_INT_REGISTER));
            return false;
        } break;
        case 0x6000:
        {
            // LD
            uint64_t imm_address = extract_uimm_rvc_t1(ins);
            uint16_t rvc_rd      = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1     = expand_rvc_int_register(extract_rs1_rvc(ins));

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC LD rd=%" PRIu16 " <- rs1=%" PRIu16 " + imm=%" PRIu64 "\n", rvc_rd,
                rvc_rs1, imm_address);
            #endif

            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, rvc_rs1, imm_address, rvc_rd, 8, true, MEM_TRANSACTION_NONE,
                LOAD_INT_REGISTER));
            return false;
        } break;
        case 0x8000:
        {
            // Reserved, this is a decode fault
        } break;
        case 0xA000:
        {
            // FSD
            uint16_t rvc_rs2  = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            uint32_t imm_53 = (ins & 0x1C00) >> 7;
            uint32_t imm_76 = (ins & 0x60) << 1;

            uint32_t imm = (imm_53 | imm_76);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-------> RVC FSD %" PRIu16 " -> memory[ %" PRIu16 " + %" PRIu32 " ]\n",
                rvc_rs2, rvc_rs1, imm);
            #endif

            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, rvc_rs1, imm, rvc_rs2, 8, MEM_TRANSACTION_NONE,
                STORE_FP_REGISTER));
            return false;
        } break;
        case 0xC000:
        {
            // SW
            uint16_t rvc_rs2 = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            uint32_t offset_53 = (ins & 0x1C00) >> 7;
            uint32_t offset_6  = (ins & 0x20) << 1;
            uint32_t offset_2  = (ins & 0x40) >> 4;

            uint64_t offset = (offset_2 | offset_6 | offset_53);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC SW %" PRIu16 " -> %" PRIu16 " + %" PRIu64 "\n", rvc_rs2, rvc_rs1,
                offset);
            #endif

            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, rvc_rs1, offset, rvc_rs2, 4, MEM_TRANSACTION_NONE,
                STORE_INT_REGISTER));
            return false;
        } break;
        case 0xE000:
        {
            // SD
            uint16_t rvc_rs2 = expand_rvc_int_register(extract_rs2_rvc(ins));
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            uint32_t offset_53 = (ins & 0x1C00) >> 7;
            uint32_t offset_6  = (ins & 0x20) << 1;
            uint32_t offset_7  = (ins & 0x40) << 1;

            uint64_t offset = (offset_7 | offset_6 | offset_53);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC SD %" PRIu16 " -> %" PRIu16 " + %" PRIu64 "\n", rvc_rs2, rvc_rs1,
                offset);
            #endif

            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, rvc_rs1, offset, rvc_rs2, 8, MEM_TRANSACTION_NONE,
                STORE_INT_REGISTER));
            return false;
        } break;
        }
        return true;
    } break;

    case 0x1:
    {
        const uint32_t c_func_code = ins & 0xE000;

        switch ( c_func_code ) {
        case 0x0:
        {
            // NOP // ADDI
            uint16_t rvc_rs1 = static_cast<uint16_t>((ins & 0xF80) >> 7);

            if ( 0 == rvc_rs1 ) {
                // This is a no-op?
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> creates a no-op.\n");
                #endif
                bundle->addInstruction(
                    new VanadisAddImmInstruction<int64_t>(ins_address, hw_thr, options_, 0, 0, 0));
                return false;
            }
            else {
                uint32_t imm_40 = (ins & 0x7C) >> 2;
                uint32_t imm_5  = (ins & 0x1000) >> 7;

                int64_t imm = (imm_40 | imm_5);

                if ( imm_5 != 0 ) { imm |= 0xFFFFFFFFFFFFFFC0; }

                if ( imm_5 == 0 ) {
                    // Tehcnicall this is a HINT, now what do we do?
                }

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> RVC ADDI %" PRIu16 " = %" PRIu16 " + %" PRId64 "\n", rvc_rs1,
                    rvc_rs1, imm);
                #endif
                bundle->addInstruction(
                    new VanadisAddImmInstruction<int64_t>(
                        ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, imm));
                return false;
            }
            return true;
        } break;

        case 0x2000:
        {
            // ADDIW
            uint16_t rd     = static_cast<uint16_t>((ins & 0xF80) >> 7);
            uint32_t imm_40 = (ins & 0x7C) >> 2;
            uint32_t imm_5  = (ins & 0x1000) >> 7;

            int32_t imm = (imm_40 | imm_5);

            if ( imm_5 != 0 ) { imm |= 0xFFFFFFC0; }

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "-----> RVC ADDIW  reg: %" PRIu16 ", imm=%" PRId32 "\n", rd, imm);
            #endif

            // This really is a W-clipping instruction, so make INT32
            bundle->addInstruction(new VanadisAddImmInstruction<int32_t>(
                ins_address, hw_thr, options_, rd, rd, static_cast<int32_t>(imm)));
            return false;
        } break;
        case 0x4000:
        {
            // LI
            uint16_t rd     = static_cast<uint16_t>((ins & 0xF80) >> 7);
            uint32_t imm_40 = (ins & 0x7C) >> 2;
            uint32_t imm_5  = (ins & 0x1000) >> 7;

            int64_t imm = (imm_40 | imm_5);

            if ( imm_5 != 0 ) { imm |= 0xFFFFFFFFFFFFFFC0; }

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC load imediate (LI) reg: %" PRIu16 ", imm=%" PRId64 "\n", rd, imm);
            #endif

            bundle->addInstruction(
                new VanadisSetRegisterInstruction<int64_t>(
                    ins_address, hw_thr, options_, rd, imm));
            return false;
        } break;
        case 0x6000:
        {
            // ADDI16SP / LUI
            uint16_t rd = static_cast<uint16_t>((ins & 0xF80) >> 7);

            if ( rd != 2 ) {
                // LUI
                uint32_t imm_1612 = (ins & 0x7C) << 10;
                uint32_t imm_17   = (ins & 0x1000) << 5;

                int32_t imm = (imm_17 | imm_1612);

                        if(imm_17 != 0) {
                            imm = imm | 0xFFFC0000;
                        }

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0,
                    "-----> RVC load imediate (LUI) reg: %" PRIu16 ", imm=%" PRId32 " (%#" PRIx32 ")\n", rd, imm,
                    imm);
                #endif

                bundle->addInstruction(
                    new VanadisSetRegisterInstruction<int32_t>(
                        ins_address, hw_thr, options_, rd, imm));
                return false;
            }
            else {
                // ADDI16SP
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "----> RVC ADDI16SP\n");
                #endif
                const uint32_t imm_5  = (ins & 0x4) << 3;
                const uint32_t imm_87 = (ins & 0x18) << 4;
                const uint32_t imm_6  = (ins & 0x20) << 1;
                const uint32_t imm_4  = (ins & 0x40) >> 2;
                const uint32_t imm_9  = (ins & 0x1000) >> 3;

                int64_t imm = imm_5 | imm_87 | imm_6 | imm_4 | imm_9;

                if ( imm_9 != 0 ) { imm |= 0xFFFFFFFFFFFFFC00; }

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> RVC ADDI16SP r2 <- %" PRId64 "\n", imm);
                #endif
                bundle->addInstruction(new VanadisAddImmInstruction<int64_t>(ins_address, hw_thr, options_, 2, 2, imm));
                return false;
            }
        } break;
        case 0x8000:
        {
            const uint32_t c_func2_code = ins & 0xC00;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(CALL_INFO, 16, 0, "------> RCV arith code: %" PRIu32 "\n", c_func2_code);
            #endif
            switch ( c_func2_code ) {
            case 0x0:
            {
                // SRLI
                uint32_t shamt_5  = (ins & 0x1000) >> 7;
                uint32_t shamt_40 = (ins & 0x7C) >> 2;

                uint64_t shift_by = shamt_5 | shamt_40;

                uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "--------> RVC SRLI %" PRIu16 " = %" PRIu16 " >> %" PRIu64 " (0x%" PRI_ADDR ")\n",
                    rvc_rs1, rvc_rs1, shift_by, shift_by);
                #endif
                bundle->addInstruction(
                    new VanadisShiftRightLogicalImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, shift_by));
                return false;
            } break;
            case 0x400:
            {
                // SRAI
                uint32_t shamt_5  = (ins & 0x1000) >> 7;
                uint32_t shamt_40 = (ins & 0x7C) >> 2;

                uint64_t shift_by = shamt_5 | shamt_40;

                uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "--------> RVC SRAI %" PRIu16 " = %" PRIu16 " >> %" PRIu64 " (0x%" PRI_ADDR ")\n",
                    rvc_rs1, rvc_rs1, shift_by, shift_by);
                #endif
                bundle->addInstruction(
                    new VanadisShiftRightArithmeticImmInstruction<VanadisRegisterFormat::VANADIS_FORMAT_INT64>(
                        ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, shift_by));
                return false;
            } break;
            case 0x800:
            {
                // ANDI
                uint32_t imm_5  = (ins & 0x1000) >> 7;
                uint32_t imm_40 = (ins & 0x7C) >> 2;

                uint64_t imm = imm_5 | imm_40;

                if ( imm_5 != 0 ) { imm |= 0xFFFFFFFFFFFFFFC0; }

                uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "--------> RVC ANDI %" PRIu16 " = %" PRIu16 " & %" PRIu64 " (0x%" PRI_ADDR ")\n",
                    rvc_rs1, rvc_rs1, imm, imm);
                #endif

                bundle->addInstruction(
                    new VanadisAndImmInstruction(ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, imm));
                return false;
            } break;
            case 0xC00:
            {
                // Arith
                uint16_t rvc_rs2 = expand_rvc_int_register(extract_rs2_rvc(ins));
                uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                switch ( ins & 0x1000 ) {
                case 0x0:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "------> RVC arith check zero family\n");
                    #endif

                    switch ( ins & 0x60 ) {
                    case 0x0:
                    {
                        // SUB
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "--------> RVC SUB %" PRIu16 " <- %" PRIu16 " - %" PRIu16 "\n",
                            rvc_rs1, rvc_rs1, rvc_rs2);
                        #endif
                        bundle->addInstruction(
                            new VanadisSubInstruction<int64_t>(
                                ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2, true));
                        return false;
                    } break;
                    case 0x20:
                    {
                        // XOR
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "--------> RVC XOR %" PRIu16 " <- %" PRIu16 " ^ %" PRIu16 "\n",
                            rvc_rs1, rvc_rs1, rvc_rs2);
                        #endif
                        bundle->addInstruction(
                            new VanadisXorInstruction(ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2));
                        return false;
                    } break;
                    case 0x40:
                    {
                        // OR
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "--------> RVC OR %" PRIu16 " <- %" PRIu16 " | %" PRIu16 "\n",
                            rvc_rs1, rvc_rs1, rvc_rs2);
                        #endif
                        bundle->addInstruction(
                            new VanadisOrInstruction(ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2));
                        return false;
                    } break;
                    case 0x60:
                    {
                        // AND
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "--------> RVC AND %" PRIu16 " <- %" PRIu16 " & %" PRIu16 "\n",
                            rvc_rs1, rvc_rs1, rvc_rs2);
                        #endif

                        bundle->addInstruction(
                            new VanadisAndInstruction(ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2));
                        return false;
                    } break;
                    }
                } break;
                case 0x1000:
                {
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(CALL_INFO, 16, 0, "------> RVC arith check one family\n");
                    #endif

                    switch ( ins & 0x60 ) {
                    case 0x0:
                    {
                        // SUBW
                        uint16_t rvc_rs2 = expand_rvc_int_register(extract_rs2_rvc(ins));
                        uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "------> RVC SUBW %" PRIu16 " <- %" PRIu16 " + %" PRIu16 "\n",
                            rvc_rs1, rvc_rs1, rvc_rs2);
                        #endif

                        bundle->addInstruction(
                            new VanadisSubInstruction<int32_t>(
                                ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2, false));
                        return false;

                    } break;
                    case 0x20:
                    {
                        // ADDW
                        uint16_t rvc_rs2 = expand_rvc_int_register(extract_rs2_rvc(ins));
                        uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "------> RVC ADDW %" PRIu16 " <- %" PRIu16 " + %" PRIu16 " hw_thr=%d\n",
                            rvc_rs1, rvc_rs1, rvc_rs2,hw_thr);
                        #endif
                        bundle->addInstruction(
                            new VanadisAddInstruction<int32_t>(
                                ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, rvc_rs2));
                        return false;

                    } break;
                    case 0x40:
                    {
                        // Reserved, not used
                    } break;
                    case 0x60:
                    {
                        // Reserved, not used
                    } break;
                    }
                } break;
                }
            } break;
            }
        } break;
        case 0xA000:
        {
            // JUMP
            const uint32_t imm_11 = (ins & 0x1000) >> 1;
            const uint32_t imm_4  = (ins & 0x800) >> 7;
            const uint32_t imm_98 = (ins & 0x600) >> 1;
            const uint32_t imm_10 = (ins & 0x100) << 2;
            const uint32_t imm_6  = (ins & 0x80) >> 1;
            const uint32_t imm_7  = (ins & 0x40) << 1;
            const uint32_t imm_31 = (ins & 0x38) >> 2;
            const uint32_t imm_5  = (ins & 0x4) << 3;

            int64_t imm_final =
                static_cast<int64_t>(imm_11 | imm_4 | imm_98 | imm_10 | imm_6 | imm_7 | imm_31 | imm_5);

            // sign-extend the immediate if needed (is MSB 1, indicates negative)
            if ( imm_11 != 0 ) { imm_final |= 0xFFFFFFFFFFFFF800; }

            const int64_t  pc_i64      = static_cast<int64_t>(ins_address);
            const uint64_t jump_target = static_cast<uint64_t>(pc_i64 + imm_final);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "----> decode RVC JUMP pc=0x%" PRI_ADDR " + imm=%" PRId64 " = 0x%" PRI_ADDR "\n", ins_address,
                imm_final, jump_target);
            #endif

            bundle->addInstruction(new VanadisJumpInstruction(
                ins_address, hw_thr, options_, 2, jump_target, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 0xC000:
        {
            // BEQZ
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            const uint32_t imm_8  = (ins & 0x1000) >> 4;
            const uint32_t imm_43 = (ins & 0xC00) >> 7;
            const uint32_t imm_76 = (ins & 0x60) << 1;
            const uint32_t imm_21 = (ins & 0x18) >> 2;
            const uint32_t imm_5  = (ins & 0x4) << 3;

            int64_t imm_final = static_cast<int64_t>(imm_8 | imm_76 | imm_5 | imm_43 | imm_21);

            if ( imm_8 != 0 ) { imm_final |= 0xFFFFFFFFFFFFFE00; }

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "----> decode RVC BEQZ %" PRIu16 " jump to: 0x%" PRI_ADDR " + 0x%" PRI_ADDR " = 0x%" PRI_ADDR "\n",
                rvc_rs1, ins_address, imm_final, ins_address + imm_final);
            #endif

            bundle->addInstruction(new VanadisBranchRegCompareImmInstruction<
                                    int64_t, REG_COMPARE_EQ>(
                ins_address, hw_thr, options_, 2, rvc_rs1, 0, imm_final, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        case 0xE000:
        {
            // BNEZ
            uint16_t rvc_rs1 = expand_rvc_int_register(extract_rs1_rvc(ins));

            const uint32_t imm_8  = (ins & 0x1000) >> 4;
            const uint32_t imm_43 = (ins & 0xC00) >> 7;
            const uint32_t imm_76 = (ins & 0x60) << 1;
            const uint32_t imm_21 = (ins & 0x18) >> 2;
            const uint32_t imm_5  = (ins & 0x4) << 3;

            int64_t imm_final = static_cast<int64_t>(imm_8 | imm_76 | imm_5 | imm_43 | imm_21);

            if ( imm_8 != 0 ) { imm_final |= 0xFFFFFFFFFFFFFE00; }

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "----> decode RVC BNEZ %" PRIu16 " jump to: 0x%" PRI_ADDR " + 0x%" PRI_ADDR " = 0x%" PRI_ADDR "\n",
                rvc_rs1, ins_address, imm_final, ins_address + imm_final);
            #endif

            bundle->addInstruction(new VanadisBranchRegCompareImmInstruction<
                                    int64_t, REG_COMPARE_NEQ>(
                ins_address, hw_thr, options_, 2, rvc_rs1, 0, imm_final, VANADIS_NO_DELAY_SLOT));
            return false;
        } break;
        }
        return true;
    } break;

    case 0x2:
    {
        const uint32_t c_func_code = ins & 0xE000;

        #ifdef VANADIS_BUILD_DEBUG
        output_->verbose(
            CALL_INFO, 16, 0, "---> RVC function code = %" PRIu32 " / 0x%" PRIx32 "\n", c_func_code, c_func_code);
        #endif

        switch ( c_func_code ) {
        case 0x0:
        {
            // SLLI
            uint32_t shamt_5  = (ins & 0x1000) >> 7;
            uint32_t shamt_40 = (ins & 0x7C) >> 2;

            uint64_t shift_by = shamt_5 | shamt_40;

            uint16_t rvc_rs1 = (ins & 0xF80) >> 7;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "--------> RVC SLLI %" PRIu16 " = %" PRIu16 " >> %" PRIu64 " (0x%" PRIx64 ")\n",
                rvc_rs1, rvc_rs1, shift_by, shift_by);
            #endif
            bundle->addInstruction(
                new VanadisShiftLeftLogicalImmInstruction<uint64_t>(
                    ins_address, hw_thr, options_, rvc_rs1, rvc_rs1, shift_by));
            return false;

        } break;
        case 0x2000:
        {
            // FLDSP
            uint32_t imm_86 = (ins & 0x1C) << 4;
            uint32_t imm_43 = (ins & 0x60) >> 2;
            uint32_t imm_5  = (ins & 0x1000) >> 7;

            uint32_t offset = imm_5 | imm_43 | imm_86;

            uint16_t rvc_rd = static_cast<uint16_t>((ins & 0xF80) >> 7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "--------> RVC FLDSP %" PRIu16 " <- memory[sp (r2) + %" PRIu32 "]\n", rvc_rd,
                offset);
            #endif
            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, 2, offset, rvc_rd, 8, false, MEM_TRANSACTION_NONE,
                LOAD_FP_REGISTER));
            return false;

        } break;
        case 0x4000:
        {
            // LWSP
            uint32_t imm_76 = (ins & 0xC) << 4;
            uint32_t imm_42 = (ins & 0x70) >> 2;
            uint32_t imm_5  = (ins & 0x1000) >> 7;

            uint32_t offset = imm_5 | imm_42 | imm_76;

            uint16_t rvc_rd = static_cast<uint16_t>((ins & 0xF80) >> 7);

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "--------> RVC LWSP %" PRIu16 " <- memory[sp (r2) + %" PRIu32 "]\n", rvc_rd,
                offset);
            #endif

            bundle->addInstruction(new VanadisLoadInstruction(
                ins_address, hw_thr, options_, 2, offset, rvc_rd, 4, true, MEM_TRANSACTION_NONE,
                LOAD_INT_REGISTER));
            return false;

        } break;
        case 0x6000:
        {
            // FLWSP
            uint16_t rvc_rd = (ins & 0xF80) >> 7;

            if ( 0 == rvc_rd ) {
                // FLWSP
                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(CALL_INFO, 16, 0, "-----> RVC FLWSP STOP\n");
                #endif
            }
            else {
                // LDSP
                uint32_t offset_86 = (ins & 0x1C) << 4;
                uint32_t offset_43 = (ins & 0x60) >> 2;
                uint32_t offset_5  = (ins & 0x1000) >> 7;

                int64_t offset = offset_43 | offset_5 | offset_86;

                #ifdef VANADIS_BUILD_DEBUG
                output_->verbose(
                    CALL_INFO, 16, 0, "-----> RVC LDSP rd=%" PRIu16 " <- %d + %" PRId64 "\n", rvc_rd,
                    2, offset);
                #endif

                bundle->addInstruction(new VanadisLoadInstruction(
                    ins_address, hw_thr, options_, 2, offset, rvc_rd, 8, true, MEM_TRANSACTION_NONE,
                    LOAD_INT_REGISTER));
                return false;
            }
        } break;
        case 0x8000:
        {
            // Jumps
            const uint32_t c_func_12     = ins & 0x1000;
            const uint32_t c_func_12_rs2 = (ins & 0x7C) >> 2;
            const uint32_t c_func_12_rs1 = (ins & 0xF80) >> 7;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "------> RVC 12b: %" PRIu32 " / rs1=%" PRIu32 " / rs2=%" PRIu32 "\n",
                c_func_12, c_func_12_rs1, c_func_12_rs2);
            #endif

            if ( c_func_12 == 0 ) {
                if ( c_func_12_rs2 == 0 ) {
                    // JR unless rs1 = 0
                    #ifdef VANADIS_BUILD_DEBUG
                    output_->verbose(
                        CALL_INFO, 16, 0, "--------> (generates) RVC JR %" PRIu32 "\n", c_func_12_rs1);
                    #endif

                    bundle->addInstruction(new VanadisJumpRegInstruction(
                        ins_address, hw_thr, options_, 2, c_func_12_rs1, VANADIS_NO_DELAY_SLOT));
                    return false;
                }
                else {
                    if ( c_func_12_rs1 == 0 ) {
                        // HINT (turns into a NOP?)
                    }
                    else {
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0, "--------> (generates) RVC MV %" PRIu32 " <- %" PRIu32 " hw_thr=%d\n",
                            c_func_12_rs1, c_func_12_rs2, hw_thr);
                        #endif
                        bundle->addInstruction(
                            new VanadisAddInstruction<int64_t>(
                                ins_address, hw_thr, options_, static_cast<uint16_t>(c_func_12_rs1),
                                static_cast<uint16_t>(c_func_12_rs2), options_->getRegisterIgnoreWrites()));

                        return false;
                    }
                }
            }
            else {
                if ( c_func_12_rs2 == 0 ) {
                    if ( c_func_12_rs1 != 0 ) {
                        // RVC JALR
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0,
                            "--------> (generates) RVC JALR link-reg: 1 / jump-to: %" PRIu16 "\n",
                            static_cast<uint16_t>(c_func_12_rs1));
                        #endif

                        bundle->addInstruction(new VanadisJumpRegLinkInstruction(
                            ins_address, hw_thr, options_, 2, 1, c_func_12_rs1, 0, VANADIS_NO_DELAY_SLOT));
                        return false;
                    }
                    else {
                        // RVC EBREAK - fault?
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0,
                            "--------> (generates) RVC EBREAK -> pipeline fault if executed\n");
                        #endif
                        bundle->addInstruction(new VanadisInstructionFault(
                            ins_address, hw_thr, options_, "EBREAK executed, pipeline halt/stop.\n"));
                        return false;
                    }
                }
                else {
                    if ( c_func_12_rs1 == 0 ) {
                        // RVC ADD HINT - fault?
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(CALL_INFO, 16, 0, "--------> (generates) RVC ADD HINT hw_thr=%d\n", hw_thr);
                        #endif
                        bundle->addInstruction(
                            new VanadisAddInstruction<int64_t>(
                                ins_address, hw_thr, options_, 0, 0, 0));
                        return false;
                    }
                    else {
                        // ADD
                        #ifdef VANADIS_BUILD_DEBUG
                        output_->verbose(
                            CALL_INFO, 16, 0,
                            "--------> (generates) RVC ADD %" PRIu32 " <- %" PRIu32 " + %" PRIu32 " hw_thr=%d\n",
                            c_func_12_rs1, c_func_12_rs1, c_func_12_rs2, hw_thr);
                        #endif
                        bundle->addInstruction(
                            new VanadisAddInstruction<int64_t>(
                                ins_address, hw_thr, options_, static_cast<uint16_t>(c_func_12_rs1),
                                static_cast<uint16_t>(c_func_12_rs1), static_cast<uint16_t>(c_func_12_rs2)));
                        return false;
                    }
                }
            }
        } break;
        case 0xA000:
        {
            // FSDSP
            uint16_t rvc_src = (ins & 0x7C) >> 2;

            uint32_t offset_53 = (ins & 0x1C00) >> 7;
            uint32_t offset_86 = (ins & 0x380) >> 1;

            uint32_t offset = offset_86 | offset_53;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC FSDSP %" PRIu16 " -> memory[ sp + %" PRIu32 " ]\n", rvc_src, offset);
            #endif
            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, 2, offset, rvc_src, 8, MEM_TRANSACTION_NONE, STORE_FP_REGISTER));
            return false;
        } break;
        case 0xC000:
        {
            // SWSP
            uint16_t rvc_src = (ins & 0x7C) >> 2;

            uint32_t offset_52 = (ins & 0x1E00) >> 7;
            uint32_t offset_76 = (ins & 0x180) >> 1;

            uint32_t offset = offset_76 | offset_52;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC SWSP %" PRIu16 " -> %" PRIu16 " + %" PRIu32 "\n", rvc_src, (uint16_t)2,
                offset);
            #endif

            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, 2, offset, rvc_src, 4, MEM_TRANSACTION_NONE, STORE_INT_REGISTER));
            return false;
        } break;
        case 0xE000:
        {
            // SDSP
            uint16_t rvc_src = (ins & 0x7C) >> 2;

            uint32_t offset_53 = (ins & 0x1C00) >> 7;
            uint32_t offset_86 = (ins & 0x380) >> 1;

            uint32_t offset = offset_86 | offset_53;

            #ifdef VANADIS_BUILD_DEBUG
            output_->verbose(
                CALL_INFO, 16, 0, "-----> RVC SDSP %" PRIu16 " -> %" PRIu16 " + %" PRIu32 "\n", rvc_src, (uint16_t)2, offset);
            #endif

            bundle->addInstruction(new VanadisStoreInstruction(
                ins_address, hw_thr, options_, 2, offset, rvc_src, 8, MEM_TRANSACTION_NONE, STORE_INT_REGISTER));
            return false;
        } break;
        }
        return true;
    } break;
    default:
        return true;
    }

    return true; // If we made it here, we didn't successfully decode
}

